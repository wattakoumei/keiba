...
決定論で出すフラグ（seed だけで完結＝コース知識不要・I10 のワンソース seed 群に属す）:
  ...
  - class_jump 大幅昇級（class_move）    現級比+2以上=-0.5
...
DIST_SHIFT  = 400   # m。前走比これ以上の距離替わりで減点（適性の再現性に不安）
...
def assess_horse(h, race_date, race_dist=None):
    ...
    vs = cm.get("vs_current")
    if isinstance(vs, (int, float)) and vs <= -CLASS_JUMP:   # vs_current は現級が上なら負（過去が下級）
        flags.append({"tag": "class_jump", "delta": -0.5, "note": f"現級比{abs(vs)}段の昇級＝相手強化"})

    # dist_shift: 距離大幅替わり（前走距離 vs 今回距離）
    if recent and race_dist and recent[0].get("dist"):
        d = abs(race_dist - recent[0]["dist"])
        if d >= DIST_SHIFT:
            updown = "延長" if race_dist > recent[0]["dist"] else "短縮"
            flags.append({"tag": "dist_shift", "delta": -0.5,
                          "note": f"前走{recent[0]['dist']}m→今回{race_dist}m＝{d}m{updown}（適性の再現性に不安）"})
...
def self_check():
    sample = {"race_id": "20260628-x-10", "distance": 1700, "horses": [
        ...
        {"no": 4, "name": "健康馬", "sex_age": "牡5", "recent": [{"pos": 2, "field": 14, "date": "2026-06-14", "dist": 1700}], "class_move": {"vs_current": 0}},
        {"no": 5, "name": "距離短縮", "sex_age": "牡5", "recent": [{"pos": 4, "field": 14, "date": "2026-06-14", "dist": 2100}], "class_move": {}},
    ]}
    ...
    assert any(f["tag"] == "dist_shift" for f in g[5]["flags"]), "2100→1700の距離替わりが出ない"
    # %・市場語が出力に混入しないこと
