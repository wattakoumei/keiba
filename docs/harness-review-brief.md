...
## 6. すでに分かっている打ち手の候補（叩き台・専門家の判断を仰ぎたい）

| 案 | 中身 | 狙う問題 | 懸念 |
|---|---|---|---|
| C | 観点subagentの**schema強制を廃止 → research-X.jsonをWrite＋短文返し**に統一。**かつ parse/失敗時は保存済みファイルを再読込して救済**（schema除去単独でなく「保存検証＋救済」をセット） | P1（最大） | 構造化の検証が緩む（下流の合成器が壊れた要約を読むリスク）→ Gの bundle 検証で担保 |
| **G** | **`tools/validate_research_bundle.py` 新設（または `validate_report.py` 拡張）**: `used_observations` と `data/races/<id>/research-*.json` の**対応・必須充足を検証**し、欠落観点をゲートで検出（今回の `research-A.json` 欠落を捕捉） | P5・P6 | — |
| A | `tools/wf_cost_report.py` 新設：transcript dirの`agent-*.jsonl`を集計し per-agent コスト/失敗を自動出力 | P5 | — |
| B | parse失敗時に最後のtool-call生contentを保存 or 失敗エージェントをschema無しで1回再投げ | P5 | — |
| D | 各obs定義に**WebFetch回数のハードガード**（例: ≤8）＋「全頭カバーは満たしつつ重複fetch禁止」 | P3 | 取得不足で精度低下しないか |
| E | 部分失敗を**本体内で同期リトライ／縮退**（観点欠落なら明示マーク or 重要観点だけ再試行） | P2,P6 | 実装複雑度 |
| F | **観点の統合・削減**（11→7等）や seed活用拡大で WebFetch を減らす | P4 | 精度とのトレードオフ |

---

## 7. 専門家に判断・提案を仰ぎたい論点

1. **信頼性/部分失敗の設計**: LLMエージェントの fan-out で「schema強制 vs ファイル保存+短文返し」のどちらを既定にすべきか。fan-outの部分失敗を**本体内で**どう扱うべきか（同期リトライ / フォールバック / 縮退して欠落を明示）。P1/P6への構造的解。
2. **コスト削減（精度を落とさず）**: 16頭×11観点のweb I/O（特にWebFetch全文）を減らす設計。観点統合・seed活用・検索回数規律・キャッシュなど、どれが効くか。市場ゼロ（内在情報のみ）の制約は維持したい。
3. **観測可能性**: per-agentのコスト/失敗/長尺を**毎回・自動で**可視化する最小の計装（P5）。
4. **アーキテクチャ全体の妥当性**: 「観点ごと専属subagentを全並列 → バリア → 合成」という現方式自体が、この規模・制約に対して妥当か。別構成（重要観点優先の段階実行、2段階の粗→精、頭数に応じた動的縮退など）の提案があれば。
5. **規律の効かせ方**: 「完全性優先（全頭カバー）」と「ツール数の上限」は今回衝突して上限が無視された（P3）。プロンプト規律でなく**機構（ハードキャップ/予算）で縛る**べきか。
