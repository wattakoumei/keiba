...
| ◎ | **I5/全頭/順序** | `validate_report.py` 強化＝**複数パターン必須（patterns≥2）**・**全頭カバー（rank・leg_table=field_size）**・**leg_table↔rank の馬番集合一致**・**rank_order の連番**を warning→**error**。さらに **rank_order は int 必須**（null/文字列混在で `sorted()` がクラッシュせず明示エラーに）。退化・破損出力を遮断。既存2レース通過 | P2（退化/破損遮断） |
| ◎ | **docstring** | `validate_research_bundle.py` の冒頭説明を実装（部分欠損=error・E は legs 検証）に同期 | P3（説明ズレ） |
