...
STEP3 証拠ファンアウト  ★Workflowツールで実行★
        ...
        ── ★欠落ゲート（2026-06実装・必須停止）: 期待観点 − 返却観点 を検出→本体内で同期リトライ→なお欠落なら throw（部分証拠での合成を中止・合成トークンも浪費しない。当該観点を再取得して resume） ──
...
| 状態 | 案 | 中身 | 狙う問題 |
|:--:|---|---|---|
| ◎ | **G** | `tools/validate_research_bundle.py` 新設＝`used_observations`↔実 `research-<観点>.json` の**対応ゲート**。STEP5必須化。**欠落＝error／部分欠損（horses<field・E は legs<field）も error**。kawasaki=B欠落+E空+I空を検出、tokyoはA復元で全16頭充足し通過 | P5・P6 |
| ◎ | **E** | 部分失敗を**本体内で同期リトライ**＝PaceSynthesis前の欠落ゲート。**リトライ後も欠落なら throw＝必須停止**（部分証拠で合成しない・合成トークンも浪費しない／当該観点を再取得して resume＝成功分はキャッシュ再利用）。本体完了後の直列再取得も解消 | P1・P2・P6 |
| ◎ | **I5/全頭/順序** | `validate_report.py` 強化＝**複数パターン必須（patterns≥2）**・**全頭カバー（rank・leg_table=field_size）**・**leg_table↔rank の馬番集合一致**・**rank_order の連番（重複/欠番）**を warning→**error** に。退化・破損出力を遮断。既存2レースは通過 | P2（退化/破損遮断） |
