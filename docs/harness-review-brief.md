...
| ◎ | **I1スキャン** | 両validatorに **I1禁止語スキャン**（人気・オッズ・配当・払戻＋**予想印・専門紙・odds・market**）＝report/ログだけでなく **research JSON に残っても error**。**URL(http…)は引用先として走査除外**＝誤検出回避。tokyo の「N番人気」「market_excluded宣言」を除去、CREED にも「市場語は1文字も書かない・除外宣言も書かない」を明記（発生源を塞ぐ） | P1（I1漏れ） |
| ◎ | **合成前 set一致** | SKILL の合成前ゲートを **行数=field・各 no int・重複なし＋`expected_nos` との exact set 一致**に強化（別馬混入99も停止）。N行でも壊れた TOON が合成に渡るのを止める | P2 |
