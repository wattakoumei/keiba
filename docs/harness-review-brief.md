...
| 状態 | 案 | 中身 | 狙う問題 |
|:--:|---|---|---|
| ◎ | **G** | `tools/validate_research_bundle.py` 新設＝`used_observations`↔実 `research-<観点>.json` の**対応ゲート**。STEP5必須化。**欠落＝error／件数でなく `report.rank` との馬番集合一致**で検証（同数でも**重複・別馬混入で1頭欠ける**ケースを捕捉。非E=`horses[].no`/E=`legs[].no`）。kawasaki=B欠落+E空+I空を検出、tokyoはA復元で全16頭一致し通過 | P5・P6 |
