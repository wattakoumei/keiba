...
STEP3 証拠ファンアウト  ★Workflowツールで実行★
        ...
        ── ★欠落ゲート（2026-06実装）: 期待観点 − 返却観点 を欠落として検出し本体内で同期リトライ、なお欠落なら log で明示 ──
...
>
> **対応状況（2026-06実装済み）**: 下表 案C(欠落ゲート部分)・案G・案E(同期リトライ)・I5/全頭カバーの error 化を実装し**必須ゲート化**した（§6の◎）。残る設計判断（review投影・tool予算）は §7 に集約。

## 6. 打ち手（◎=実装済み・必須ゲート化／△=未着手・要判断）

| 状態 | 案 | 中身 | 狙う問題 |
|:--:|---|---|---|
| ◎ | **G** | `tools/validate_research_bundle.py` 新設＝`used_observations`↔実 `research-<観点>.json` の**対応ゲート**。STEP5必須化。kawasaki=B欠落/tokyo=A欠落を検出（tokyoはAを復元保存して通過）。schema検証(`validate_report --all`)とは別tool＝過去レースを壊さない | P5・P6 |
| ◎ | **E** | 部分失敗を**本体内で同期リトライ**＝PaceSynthesis前の欠落ゲート（期待観点−返却観点を検出→再起動→なお欠落なら log 明示）。本体完了後の直列再取得を解消 | P2・P6 |
| ◎ | **I5/全頭** | `validate_report.py` 強化＝**複数パターン必須（patterns≥2）**と**全頭カバー（rank=field_size）**を warning→**error** に。退化出力（1パターン/頭数欠け）を遮断。既存2レースは通過 | P2（退化遮断） |
...
2. **review loop と jsonl 投影の不整合（未着手・要判断）**: review は `predictions.jsonl` の rank record で**全馬の順位相関（Spearman/Kendall）**を取る前提だが、投影器は**印持ち馬（mark≠—）だけ**を出す。全馬相関をやるなら **(a) rank record を全馬投影**するか **(b) review が `report.json` を正本として読む**か、どちらにすべきか（後者は I10「report.json 正本」と整合的だが review の入力源変更が要る）。
