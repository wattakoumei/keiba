...
| `tools/validate_screening.py` | **screening JSON のスキーマゲート**（`data/screening/<date>-<venue>.json`・依存ゼロ）。x_axis=object|null（文字列は web build クラッシュ）・絵文字象限禁止・キーwhitelist（`x_conf`/`dango`/`gap` 等のドリフト別名を検知）・ファイル名↔date/venue 突合。screen-card STEP6 の保存直後＋CI（deploy.yml）ビルド前に必須。`--all`/`--self-check`。 |
