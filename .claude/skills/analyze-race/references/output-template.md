...
| `obs_coverage` | obj[] | **観点欠落の構造化表**（2026-07-13〜）: `{id, label, status, fallback}`・`status`∈{`縮退`(seed/決定論ツールで代替), `未取得`(代替なし)}。**正本＝`tools/inject_probs.py` が STEP5 で期待セット（既定/新馬/NAR）との差分から決定論生成**＝手書きしない。欠落ゼロなら空配列。web は非空のとき**冒頭に警告バナー＋§4に表**を描画（縮退分析の印・率は確度が低いことを読者に明示） |
