...
③ netkeiba              ← 完全CSR＋規約が自動収集禁止方向 → スクレイプ対象外（WebSearch スニペット参照のみ）
```
...
- 取得は共通ヘルパー **`tools/_polite.py`** 経由（下記「取得ポリシー」の実装正本）。競馬ラボ robots.txt は `/db/race/` を **Allow**（実行時にも `_polite` が robots を検証する）。
- **netkeiba は完全 CSR**＝urllib では空。かつ利用規約が自動収集を禁じる方向＝**スクレイプ対象にしない**（web 調査でも WebSearch スニペット経由の参照に留める）。だから②は競馬ラボ。

## 取得ポリシー（法的リスク低減・全 fetch_* 共通）

実装の正本は `tools/_polite.py`（robots・レート制限・キャッシュ・UA を一元管理）。方針:

1. **robots.txt を尊重**: ホストごとに実行時に取得・検証し、Disallow のパスは取得しない（`RefusedByRobots`）。
   取得結果の扱いは **RFC 9309 準拠**＝4xx（404等）は「制限なし」、**5xx/接続断は「全面 Disallow」扱い**（1回リトライしても取れなければ拒否→貼り付け経路へ）。**Crawl-delay / Request-rate も尊重**（上限30s）。
2. **レート制限**: 同一ホストへのリクエスト間隔は **1.5 秒以上**（人間の閲覧頻度以下。robots.txt の Crawl-delay がそれより長ければそちらに従う）。一括取得（バックフィル等）は必ず単一プロセスの逐次ループで行い、並列で同一サイトを叩かない。
   一過性の失敗（接続断・タイムアウト・HTTP 429/5xx）のリトライは `_polite` が**1回だけ**行う（Retry-After 尊重・上限30s。429以外の4xx は即エラー）。各 fetch_* は自前 retry を持たない＝二重リトライしない。
3. **キャッシュ**: 確定結果など不変ページは `data/cache/` に保存して再取得を避ける（同一URLを繰り返し叩かない）。
4. **UA**: 一般的なブラウザ UA を `_polite.UA` で一元管理（モバイル最適化/文字化け回避の互換目的。アクセス制御の回避を意図しない＝負荷対策の本質は間隔・キャッシュ・取得ページの最小化）。
5. **対象と用途の限定**: 取得するのは公開ページの事実（出走表・結果・払戻・調教時計）のみ。取得データの再配布はしない（本ハーネス内の分析・検証にのみ使う）。
6. **サイト方針の尊重**: 利用規約が自動収集を禁じる方向のサイト（netkeiba 等）はスクレイプ対象にしない。ツールの対象は JRA公式・競馬ラボ・競馬ブック（いずれも静的公開ページ・上記の礼儀装備つき）。
...
## 既知の制約
- **netkeiba は使えない**（完全 CSR＋規約）。WebSearch スニペット経由の参照に留める。
...
`race` コマンドの JRA 経路は、各馬の近走 `recent[]` に**レース特定情報**（`date` ISO・`venue`・`race`・`pos`着順・**`jockey`前走騎手**）を持ち、
同一過去レースに今回の出走馬が2頭以上出ていた事実を **`h2h`** として返す（`compute_h2h`）:
- **`recent[].jockey`（前走騎手）**＝同じ past セルから追加 fetch 無しで取得。観点K が今走鞍上と突き合わせて**乗替方向（強化/弱化/継続/テン乗り）を機械判定**＝前走騎手の web 同定を不要にする（K の最大の遅延源を除去）。鞍上履歴（2走前以前）も `recent[1..].jockey` で主戦/テン乗りの裏取りに使える。
- **`recent[].time`（走破時計）・`recent[].dist`（距離）・`recent[].body_weight`（馬体重）**＝同じ past セルから追加 fetch 無しで取得（`<p class="time">`・`<span class="dist">`・`<p class="h_weight">`）。観点A が時計の一次事実に、D が距離替わり/同コース実績に、G が過去馬体重トレンドに、`risk_flags.py` が距離替わり判定に使う＝全頭の生タイム/距離/体重を web で追う作業を消す（前走騎手と同型）。**web は事実の取得でなく解釈（レースレベル・馬場・仕上げ）に絞る。**
...
```
pbpaste | python3 tools/fetch_oikiri.py paste [--date M/D] [--json]   # 正規経路（人間がページをコピー）
python3 tools/fetch_oikiri.py week [--date M/D] [--json]              # robots が許す場合のみ
```

- 競馬ブック ベスト調教（`p.keibabook.co.jp/cyuou/bestcyokyo`）の
  **今週の好時計ランキング**。各馬: 馬名・条件・出走レース・調教日・コース(栗東ＣＷ等)・馬場・各ハロン累計・**ラスト1F**・脚色(馬なり/強め/一杯+余力)。
- **取得ポリシー（2026-07 確認）**: 競馬ブック robots.txt は**全botに `Disallow: /`** → `week`（自動取得）は
  `_polite` が拒否する（設計どおり）。**人間がブラウザで開いてコピー→`paste` が正規経路**。seed 無しなら F が web 補完（不在≠調教不良）。
