...
> 予測は馬の内在情報のみで完結する。
>
> **論理ファーストの原則（v5.0／v5.1で率2カラム例外）**: ユーザーは**表しか見ない**。レポート**散文に % は出さない**（発揮能力・条件付き%等は撤廃）。**例外＝単勝率/複勝率の2カラム**（`rank[].win_prob`/`place_prob`・源は `tools/score_race.py`・web が×100表示）。
> 着順の並びは**論理**（相変位再帰の因果＝序盤A_early→中盤A_cruise→終盤A_finish）で決め、**印 ◎◯▲△× と行順**で強弱を伝える。率2カラムはその並びに**添える参考**（率順に並べ替えない）。
> 数字の代わりに **展開感度・好材料・懸念点（観点タグ＋なぜそう読んだか）** を厚く書く。`tools/score_race.py` は**単勝率/複勝率の源**（`tools/inject_probs.py` 経由で毎回 rank に注入）＋並びの整合サニティ。
...
- `scoring-model.md` — **相変位再帰の因果骨格**（相別能力 A_early/cruise/finish/class → 3相再帰）の定義。v5.1 で**単勝率/複勝率の源**に昇格（`inject_probs.py` 経由で rank に注入）＋並びの整合サニティ（並びは論理が主）
...
- `debut-catalog.md` — **新馬カタログ**（①厩舎の新馬仕込み型 ②種牡馬の初戦適性 ③セリ/募集価格の読み方 ④新馬の一般読み筋）。**新馬モード専用**＝M/C に spawn 注入（半静的・lazy 蓄積）
...
>
> **新馬戦の検出（レース特定と同時）**: レース名/条件に「新馬」「メイクデビュー」があれば**新馬モード**（STEP2）で組む。seed の `recent[]` は全馬空＝正常（risk_flags は空を返す・oikiri seed と weight_adjust は通常どおり有効）。`--class` は `2歳新馬` を渡す。
...
**併せて seed JSON を `data/races/<race-id>/seed.json` に保存する**（`fetch_racecard.py race … --json` の出力をそのまま）＝STEP5 の `assemble_report.py` がスパイン・前走騎手・走破時計・距離・馬体重の源として読む（report.json 骨格生成の入力）。

**追い切り好時計 seed の取得（観点F用・任意）**: 競馬ブック robots.txt は全bot Disallow（2026-07確認）＝自動取得 `week` は _polite が拒否する。**ユーザーがベスト調教ページをコピーできる場合のみ `pbpaste | python3 tools/fetch_oikiri.py paste --json`** で seed 化し、STEP3 で F に `oikiri` として渡す（配線は下記 Workflow 骨子）。
**全頭でない好時計の上位抜粋**＝対象馬が載れば一次事実、不在馬は F が web 補完（不在≠調教不良）。**seed が無ければ空 `[]` で渡し、F は全頭 web 調査**にフォールバック（seed は fast-path であって必須依存ではない）。`--date <M/D>` で出走日に絞れる。

**重量重みづけ seed の取得（観点D/I/G＋展開合成用・決定論）**: `出走表.md` 保存後に **`python3 tools/weight_adjust.py data/races/<race-id>/出走表.md --json` を実行**し、出力を STEP3 で D/I/G と PaceSynthesis に `weight` として渡す。
- 斤量・馬格(馬体重)×芝ダ・距離・馬場 から各馬の**符号付き重量タグ**（pace=斤量×先行→共倒れ寄与／I=斤量増の減点／D/G=馬格×馬場のパワー適性）と **`front_verdict`（先行勢の斤量から共倒れ確率を機械算定）** を返す。**web 調査ゼロ＝seed と条件だけで決まる**（[harness-invariants](../../rules/harness-invariants.md) I10 のワンソース seed 群に属す）。
- **馬場は分析時点で未確定なら `--going` 省略（良想定）**、馬体重も当日発表＝省略可（per-horse の馬格タグは保留・`body_weight_status` に明示）。当日に `--going <馬場> --weights <no:kg,...>` で再実行し**当日可変**で更新（STEP6）。
- 確定部（斤量×芝ダ×距離）は分析時に算出済み＝**§2「先行勢の質」と §3 に先取り**（後付けにしない＝I6）。坂・小回りの増幅は course-geometry を持つ PaceSynthesis 側で適用（ツールは持たない＝I10）。

**観点Iの決定論リスク seed の取得（決定論・web再調査しない）**: seed 取得後に **`python3 tools/fetch_racecard.py race <rid> --json | python3 tools/risk_flags.py - --race-date <YYYYMMDD>`**（または seed を保存して `risk_flags.py <seed.json>`）を実行し、出力を STEP3 で I に `risk` として渡す。
- seed の性齢・recent 着順率・class_move・前走間隔から **高齢 old／下降基調 decline／大幅昇級 class_jump／休み明け layoff** を符号付きで算定（コース非依存・斤量は weight が担当＝二重化しない・不利枠は course-geometry の領域で除外）。**web 調査ゼロ**＝[harness-invariants](../../rules/harness-invariants.md) I10 のワンソース seed 群。
- これは Iの**一次フラグ**＝obs-i が web の文脈で上書き可（敗因が距離/展開なら割引）。決定論層をエージェントの外に出すことで、Iの遅延源（全頭の着順履歴を web で追う作業）を消す。
...
- 当日の馬場・クッション値・含水率・風（向き・強さ）・パドック・馬体重・前半参考Rの観察値は**常に当日記入**（分析時点では本質的に未知）。
...
`observation-points.md` に従い、**STEP1 で選んだ深掘り/速報**と頭数・情報量で観点数を決める（既定 11 観点 A〜I, K, L。新馬戦は専用セット＝下記。市場Jは無い）。**速度・トークン優先＝web情報が薄い/少頭数のレースは迷ったら絞る側に倒す**：

**新馬モード（新馬戦＝過去走ゼロ・頭数に関わらずこのセット）**：
- **観点は 6 個固定 `C,E,F,I,K,M`**（＋当日再分析時のみ H）。**A/B/D/G/L は起動しない**＝時計・近走・コース実績・ローテ・条件実績が構造的に存在せず全馬スコア0の空振りになる（コース形状は従来どおり E と展開合成へ注入）。
- 各観点は agent 定義内の**「新馬戦モード」節**で動く（spawn プロンプトに新馬フラグを1行入れて確実に発動させる）。**M（厩舎新馬型・初戦準備度）は新馬専用の新設観点**＝`debut-catalog.md` ①③ の該当行を spawn 注入（C には ② 初戦適性行を追加注入）。
- 展開は全馬推定＝確信度が構造的に低い。合成は `pace-synthesis.md` の**新馬節**に従う（本線に寄せすぎない・出遅れ/行きたがり分岐を1本残す）。
- report.json に **`"debut": true`** を持たせる（`inject_probs.py` が新馬プロファイル＝score_race `profile:"debut"` を自動適用。レース名の「新馬」でも自動判定されるが明示が確実）。
...
- **web は1観点あたり 1 バッチ厳守**（CREED の deadline 条項。超過は打切る）。研究 subagent は**軽量モデル（`opts.model:'sonnet'`）**だが、**これは速報固有でなく常時**（収集は機械作業でモデル差が出ない／合成 PaceSynthesis・着順は Opus 据え置き）。**速報の壁時計レバーは観点数とバッチ数**＝モデルでない。
...
**H（当日気配・パドック）は分析が前日以前なら起動しない**＝パドック/返し馬/直前気配は当日にしか取れず、事前分析では全馬スコア0の空振りになりつつ wave1 の枠を1つ占有するだけ（前回 H は全馬0）。事前は H を外した分（深掘り最大10観点）で組み、**当日朝の再分析（STEP6 当日可変）でのみ H を追加**する。関係者コメントは F/K/I が拾うので事前でも欠落しない。締切が当日で H が取れる場合だけ事前から入れてよい。
...
- **スクレイパ seed**（STEP1 の `fetch_racecard.py` 出力）。JRA経路なら 脚質(通過順由来の精密)・テン速・血統(父母父)・枠/馬番・性齢・斤量・騎手・**h2h直接対戦**・**クラス序列タグ（`recent[].class_rank`／各馬 `class_move`／`race_class_rank`＝昇降級が機械判定済み・観点B用）**・**前走騎手（`recent[].jockey`＝乗替方向が機械判定済み・観点K用）**・**走破時計/距離/馬体重（`recent[].time`/`dist`/`body_weight`＝同じpastセルから追加fetch無し。A時計・D距離替わり・G馬体トレンドの一次事実）**、競馬ラボ経路なら脚質傾向(粗)・血統・枠。観点E/C/B/K/A/D/G はこれを **検証・補強**し、ゼロから取り直さない（`references/scraping.md`）
...
- **新馬カタログ**（新馬モードのみ・`references/debut-catalog.md`）。M に①厩舎新馬型＋③セリ価格原則、C に②初戦適性の該当行。カタログ外の厩舎/種牡馬は暫定ラベル＋追記候補で報告
- **追い切り好時計 seed**（`fetch_oikiri.py paste`＝ユーザー貼り付け経由の競馬ブック好時計。無ければ `[]`）。F に渡す。**全頭ではない上位抜粋**＝対象馬が載れば一次事実、不在馬は F が web 補完（不在≠調教不良）
...
// args = { raceId, condition, horses, field_size, expected_nos, seed, pedigree, course, oikiri, stable, weight, risk, debut, debutCatalog, points:[{id}], mode }  ※ odds は渡さない
//   mode = '速報' | '深掘り'（STEP1 の締切確認で選択）。速報は points=[A,B,D,E,I]・web1バッチ厳守（研究agentは常時 Sonnet＝モード非依存。Opusは合成 PaceSynthesis/着順だけ）
//   debut = 新馬戦フラグ（true なら points=[C,E,F,I,K,M]＝A/B/D/G/L を起動しない・各agentの「新馬戦モード」節を発動）
//   debutCatalog = debut-catalog.md の該当行（M に①厩舎新馬型＋③セリ価格原則、C に②初戦適性。新馬時のみ）
...
//   weight=weight_adjust.py の出力(斤量・馬格×馬場の符号付き重量タグ＋front_verdict)。D/I/G が per-horse タグを、PaceSynthesis が front_verdict を使う。決定論＝再調査しない
//   risk=risk_flags.py の出力(観点Iの非重量・コース非依存の決定論リスク＝old/decline/class_jump/layoff)。I が一次フラグに採用しwebは非決定論層だけ調べる。決定論＝再調査しない
//   観点の手順/ソース/スコア指針は .claude/agents/obs-<id>.md（subagent）に内蔵。ここで渡すのはデータ＋共通鉄則のみ
//   seed = fetch_racecard.py の race 出力（脚質・血統・枠/馬番・recent[]に jockey/time/dist/body_weight/venue/pos/agari）。E/C/B/K/A/D/G は seed を検証・補強する
...
const AGENT_OF = { A:'obs-a-index', B:'obs-b-recent', C:'obs-c-pedigree', D:'obs-d-aptitude', E:'obs-e-pace',
                   ...
                   L:'obs-l-repeater', M:'obs-m-debut',    // M は新馬モード専用（debut=true のときだけ points に入る）
                   N:'obs-n-class' }                       // N は NAR モード専用（/analyze-race-nar が points に入れる）
...
const HEAVY_ORDER = ['A','B','D','L','H','M','F','C','E','K','G','I']
const ordered = [...args.points].sort((a,b)=> HEAVY_ORDER.indexOf(a.id) - HEAVY_ORDER.indexOf(b.id))
const runOne = (p) => agent(
    `# レース条件\n${args.condition}\n\n# 出走馬（全頭）\n${args.horses}\n` +
    // seed を起点に検証・補強する観点: E(脚質)/C(血統)/B(近走)/K(前走騎手)/A(走破時計recent[].time)/D(コース・距離recent[].venue,dist)/G(間隔・馬体重recent[].date,body_weight)
    (['E','C','B','K','A','D','G'].includes(p.id) ? `\n# スクレイパ seed（検証・補強の起点／ゼロから取り直さない＝recent[]に time/dist/body_weight/jockey/venue/pos/agari）\n${JSON.stringify(args.seed)}\n` : ``) +
    // 新馬フラグ（各 agent 定義の「新馬戦モード」節を確実に発動させる1行）
    (args.debut ? `\n# 新馬戦（過去走なし）: あなたの agent 定義の「新馬戦モード」節に従うこと。\n` : ``) +
    // C は血統カタログ該当行、D/E はコースカタログ該当行、F は追い切り好時計 seed を渡す（カタログにある分は再調査しない）
    (p.id==='C' ? `\n# 血統カタログ該当行（pedigree-catalog.md・カタログ内は再調査しない）\n${args.pedigree||''}` : ``) +
    // 新馬モード: M に debut-catalog ①厩舎新馬型＋③セリ価格原則、C に②初戦適性の該当行を注入（カタログ内は再調査しない・外は追記候補）
    (args.debut && p.id==='M' ? `\n# 新馬カタログ該当行（debut-catalog.md ①③）\n${args.debutCatalog||''}` : ``) +
    (args.debut && p.id==='C' ? `\n# 新馬カタログ・初戦適性（debut-catalog.md ②）\n${args.debutCatalog||''}` : ``) +
    ...
    // D/I/G は weight_adjust.py の重量タグを受け取り、自分のチャンネルだけ評価に織り込む（決定論ミラー＝斤量/馬格は再調査しない）
    (['D','I','G'].includes(p.id) ? `\n# 重量重みづけ seed（weight_adjust.py・斤量/馬格×馬場の決定論タグ。自分のチャンネルを使う＝ D/G:馬格×馬場のパワー適性, I:斤量増の減点。pace タグは展開合成が使う＝ここでは無視可。斤量・馬格は再調査せずこの値を採用）\n${JSON.stringify(args.weight||{})}` : ``) +
    // I は risk_flags.py の決定論リスク（old/decline/class_jump/layoff）を一次フラグに採用し、web は非決定論層（脚部/気性/故障/中止歴）だけ調べる
    (p.id==='I' ? `\n# 決定論リスク seed（risk_flags.py・高齢/下降基調/大幅昇級/休み明け＝seedから確定。一次フラグとして採用し再調査しない。webは脚部不安/気性難/競走中止歴の実測だけに絞る。敗因が距離/展開で説明でき地力でないなら割引可）\n${JSON.stringify(args.risk||{})}` : ``) +
    `\n${CREED}${DEADLINE_CAP}\n- **全文prose は生成・保存しない**（未読のため廃止）。スキーマの構造化要約を data/races/${args.raceId}/research-${p.id}.json に保存し、同じ要約を返す。`,
    // 研究agentは常時 Sonnet（収集＝web探索＋schema抽出＝機械作業でモデル差が出ない。schema＋CREEDが構造化を担保）。
    // Opus を残すのは推論の核だけ＝PaceSynthesis(展開合成)と STEP4b 着順合成。速報の壁時計レバーは観点数とバッチ数（モデルでない）。
...
const paceModel = await agent(
  ...
  `各パターンに phase_flow{early,mid,late,result} を「序盤→行き脚→中盤→追走力→終盤→決め手→結果」の因果文で必ず著作する（I11: 散文に内部変数名 A_early/A_cruise/A_finish/A_class を使わない。行き脚/追走力/決め手/地力と書く）。` +
  `関係者の戦法宣言＞推測、近走の実脚質＞一般ラベル、先行争いが不確実なら別パターンに分岐。\n` +
  // 重量重みづけは「先行勢の質」の必須入力＝front_verdict と pace タグで共倒れ(γ系)ティアを増幅（条件付き＝重い先行が複数で競るときのみ。pace-synthesis.md「先行勢の数と質」）
  `重量重みづけ seed の front_verdict を「先行勢の質」に取り込み、先行当事者の斤量が重い（複数で競る）なら共倒れ=差し台頭パターンのティアを上げよ（線形でなく条件付き・主因は先行争いの構造）。\n# 重量重みづけ(weight_adjust.py)\n${JSON.stringify(args.weight||{})}\n` +
...
- 各パターンに **`phase_flow{early,mid,late,result}`** を著作する＝「序盤→行き脚→中盤→追走力→終盤→決め手→結果」の因果文。これが §2 の段階フロー（1行テキスト）と §3 展開感度の素。**I11: 散文に内部変数名（A_early 等）を書かない。行き脚/追走力/決め手/地力と書く。**
...
**着順の並びは論理で決める**。各馬を相変位再帰の因果（序盤の行き脚→中盤の追走力→終盤の決め手×余力、地力の変調）で相対比較し、
最有力パターンの段階フローに沿って **印 ◎◯▲△× と行順**を付ける。**% は出さない**（`scoring-model.md` の相別能力が並びの論拠の語彙）。**I11: report.json の散文には内部変数名（A_early/A_cruise/A_finish/A_class）を書かない。行き脚/追走力/決め手/地力と書く。**手順：

1. 各馬の相別能力の高低を定性で確定（観点評価＋スクレイパの `ten_speed`/`style`/`agari_best`/`recent[]` から）。序盤の速さ（行き脚）と終盤の決め手を**別々に**見て、逃げ馬と追込馬を単一尺度で比べない。
...
2c2. **◎◯は最有力パターンの riser/圏内から選ぶ（2026-07-03 25件レビュー由来・自己矛盾ガード）**: 本線が前残り系なのに差し・追込馬へ◎◯を打たない（逆も同じ）。本線 riser 外の馬に上位印を打つ場合は**「展開不問」の根拠（展開列が全パターン◎/○ 等）を pros に明記必須**。
    根拠: 自己矛盾が3レースで再発（小倉10R: α本線で差し馬◎→6着・東京12R: α本線で前崩れ狙いを◯→15着・福島3R: 展開不問◯を展開依存◎に劣後→◯1着/◎13着）。
2c3. **cons 死角の検算（2026-07-03 25件レビュー由来）**: 印を置いたら ◎〜▲ と本線中心馬の cons を読み返し、**展開・距離・消耗の死角を自分で明記していないか**を確認する。あれば (a)死角が発動するパターンのティアを1段上げる か (b)当該馬の印を1段下げる のいずれかを行う（据え置くなら理由を cons に追記）。
    根拠: cons に死角を正しく書きながら本線/◎に据えた取りこぼしが4件再発（川崎11R ◎cons[I]距離・阪神10R cons[A/I]・福島3R ◎cons[I]共倒れ・東京11R pros自身が別パターン好走を明記）。
2c4. **◯（相手筆頭）は「◎と同時に圏内」から選ぶ（2026-07-06 7/4-5レビュー由来・相方規律）**: ◯は単体の地力2番手でなく、**実現上位2ティア（本線＋対抗）のパターンで◎と同時に圏内（展開列 ◎/○）になる馬**から選ぶ。対抗以下のパターンのみで浮く差し馬を◯に置くなら「展開不問」根拠の明記必須（2c2と同じ形式）。
    根拠: 7/4-5 の14鞍で◎複勝9/14に対し「◎∧◯同時圏内」2/14＝連系・ワイドのボトルネックは相手選定。◯を本線非依存の地力差し馬に置く癖（雲仙⑩・九州S杯③・大沼⑨等）が前残り局面で全滅。2着3着の28枠中◯▲は6枠＝2列目が印に反映されていない。連系の的中は「2頭の同時分布」＝ペアで読む。
2c5. **展開列⇔印の整合検算（2026-07-11 14鞍レビュー由来）**: rank を確定する直前に、**いずれかのパターンで展開列 ○以上（`per_horse_fit` ◎/○）なのに `rank_order` が下位1/3の馬**を列挙し、1頭ずつ再検算する。据え置くなら「そのパターン実現時でもなぜ上位に来ないか」を cons に明記必須。**決定論フラグ（休み明け・下降基調等）だけを理由に fit ○以上の馬を無印最下層へ送らない**（フラグは fit 判定時に織り込み済み＝二重減点になる）。
    根拠: 自建シナリオ実現時の昇格漏れが5件再発（0705小倉10⑭ γ△発動2着・0711小倉10①「前崩れが必要」→前崩れ実現4着ro12・0711小倉12⑪ γ○→無印1着＋② γ◎なのに×→2着・0710川崎12⑥ γ○「3着拾い」→3着ro9）。
2c6. **大外×先行依存の重印検算（2026-07-11 由来）**: 本線が前残り/先行有利系で、◎〜▲を**外1/4の枠×先行・逃げ脚質**の馬に置く場合、**行き切れる根拠（テン3F最速級・鞍上の積極性・外枠からの先行実績）を pros に明記必須**。無ければ (a)印を1段下げる か (b)行き切れないパターン（出負け/被され）を1つ自建してそのfitを反映する。
    根拠: 大外先行の中心印が行き切れず沈む型が3件（0627福島12 ◎⑮失速・0704小倉10 ◎⑫行き切れず4-5番手4着・0711福島8 ▲⑮新人騎手のテン崩れ9-10→10着=本線αの先導主役ごと崩壊）。
...
4. **率注入（必須・I8）**: report.json 保存後に **`python3 tools/inject_probs.py <race-id>`** を回す＝research-*.json の観点スコア＋`pace.patterns`＋`leg_table` から score_race を決定論で回し、`win_prob`/`place_prob` を**全馬の rank[] に書き戻す**（率の手組みはしない＝再現可能・手計算の揺れを排除）。
   出力する**論理の並び vs エンジン複勝率順の食い違い**を点検シグナルとして見る＝食い違ったら**論理側を正**とし（率順に並べ替えない）、大きくズレる馬は理由を一言 `header_notes`/`engine_check` に残す（どちらかの読みに穴がある合図）。較正先(市場)の無い内在%＝**偽の精度**である旨を `header_notes` に明記する。

> 相変位再帰の因果骨格・語彙の正本は `scoring-model.md`（相別能力§2 → 3相再帰§6）。エンジンは**率(win_prob/place_prob)の源**＋並びの整合確認（並びは論理が主＝率と食い違ったら論理を正）。
> **市場・妙味・EV・Kelly・買い目は無い**（馬券は人間判断）。確信度が低い観点の影響は懸念点に明示する。

### STEP 5. 出力（構造化正本 report.json）& 自動ログ投影

**まず合成結果を `data/races/<race-id>/pace-model.json` に保存**（STEP3/4a の PaceSynthesis 戻り値）。次に **`python3 tools/assemble_report.py <race-id> --skeleton` で report.json の骨格を機械生成する**（手書きJSONをやめる）。
- ツールが埋める（決定論・手戻りの主因を構造的に潰す）: meta・`used_observations`（実在 research-*.json から）・`leg_table`・`patterns`（pace-model 転記）・`box_reverse` と `rank[].pattern_fit`（per_horse_fit 同源＝I7）・rank 骨格（スパイン＋`jockey_change` 前走騎手比較）・`pros`/`cons` の素材（research 由来を tag 付きで束ね）。市場語・%・box id・全頭カバーはツールが正しく出す。
- **人間（あなた）が論理で上書きする**（`report.skeleton.json` → `report.json` にして編集・I8 論理ファースト）: `mark`/`rank_order`（並びは論理が主＝ツールの暫定スコア降順を**論理で並べ替える**）・`intent`（F+K+H から導出）・`pace_sensitivity`（展開感度1行）・`pros`/`cons` の最終取捨と推敲・散文（`pivot`/`shape_note`/`transmission`/`pace_factors`/`verification_contract`/`day_board` の参考R/going read 等）。確定材料（枠・乗替・回避）は本文へ先取り（I6）。
- **§0 `day_board` には*分析時点で本当に未知のものだけ***（当日の参考R観察値・馬場・パドック・馬体重）。確定枠・乗替・回避は `leg_table`/`patterns`/`rank` に織り込み済み。
...
- **当日可変**: §0 ボードの当日情報（**前半参考Rで採取したバイアス**・パドック・馬場変化・当日発表の乗替/取消など）を受けたら、
  ...
  **発走前チェック必須（2026-07-11 由来）**: 朝にまとめて分析した日は、**各レースの発走前に同場の確定済みRの決着傾向（実効ペース・前残り/差し）を確認して当日可変を1回回す**（同方向2鞍以上連続で本線降格まで可＝pace-synthesis 当日可変の同項参照）。回さずに朝の分析のまま発走を迎えない。
  参考Rで「想定と馬場の質が違った」場合のみ観点D適性の読みを部分見直し。エンジンを併用する場合は新しい `prob`/`pace_level` で再実行し並びの整合だけ確認。
  当日馬場・馬体重が出たら **`python3 tools/weight_adjust.py <出走表.md> --going <馬場> --weights <no:kg,...> --json` を再実行**し、更新後の `front_verdict`／馬格タグで「先行勢の質」と D/G を再評価（道悪なら斤量負担↑・大型のパワー相殺が確定）。
...
- **論理ファースト**: レポート散文に % は出さない（例外＝単勝率/複勝率の2カラム＝`tools/inject_probs.py` で注入・並びは論理が主）。並びは論理（印＋行順）、根拠は展開感度・好材料・懸念点（観点タグ）で表す。
